...
```{r message=FALSE, echo=FALSE, warning=FALSE, cache=TRUE}
CountryColor <- colorFactor(viridis(76), top20confirmed$county)
...
leaflet(data = top20confirmed) %>% addTiles() %>% 
            ...
            addCircleMarkers(~ longitude,
                             ...
                             stroke = FALSE,
                             fillOpacity = .7,
                             fillColor = ~CountryColor(county)
            ) %>% 
            addLegend("bottomright", pal = CountryColor, values = ~county, labels = "COVID19 - Top 20 Counties")
```
